    ...
    <Dialog>
      ...
      <DialogContent>
        ...
        <fieldset><legend className="text-xs font-bold uppercase tracking-[0.14em] text-muted">Appearance</legend>
          <div className="mt-3 grid grid-cols-3 gap-2">{(["light", "dark", "system"] as Theme[]).map((theme) => <button key={theme} className={cn("rounded-xl border px-3 py-2 text-xs font-semibold capitalize", settings.theme === theme && "border-accent bg-accent-soft text-accent")} onClick={() => onChange({ theme })}>{theme}</button>)}</div>
        </fieldset>
        <fieldset className="mt-6"><legend className="text-xs font-bold uppercase tracking-[0.14em] text-muted">Text size</legend>
          <div className="mt-3 grid grid-cols-3 gap-2">{(["small", "medium", "large"] as FontSize[]).map((size) => <button key={size} className={cn("rounded-xl border px-3 py-2 text-xs font-semibold capitalize", settings.fontSize === size && "border-accent bg-accent-soft text-accent")} onClick={() => onChange({ fontSize: size })}>{size}</button>)}</div>
        </fieldset>
