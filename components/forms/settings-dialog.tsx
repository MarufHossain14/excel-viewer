  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        ...
        <fieldset><legend className="text-[12px] font-bold tracking-[-0.01em] text-muted">Appearance</legend>
          <div className="mt-3 grid grid-cols-3 gap-2">{(["light", "dark", "system"] as Theme[]).map((theme) => <button type="button" key={theme} className={cn("rounded-xl border bg-background/45 px-3 py-2.5 text-[13px] font-bold capitalize transition-colors hover:bg-accent-soft", settings.theme === theme && "border-accent bg-accent-soft text-accent shadow-sm")} onClick={() => onChange({ theme })}>{theme}</button>)}</div>
        </fieldset>
        <fieldset className="mt-6"><legend className="text-[12px] font-bold tracking-[-0.01em] text-muted">Reading size</legend>
          <div className="mt-3 grid grid-cols-3 gap-2">{(["small", "medium", "large"] as FontSize[]).map((size) => <button type="button" key={size} className={cn("rounded-xl border bg-background/45 px-3 py-2.5 text-[13px] font-bold capitalize transition-colors hover:bg-accent-soft", settings.fontSize === size && "border-accent bg-accent-soft text-accent shadow-sm")} onClick={() => onChange({ fontSize: size })}>{size}</button>)}</div>
        </fieldset>
